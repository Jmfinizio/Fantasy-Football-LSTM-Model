...
def player_projection():
    try:
        ...
        X_sequences, y_sequences, actuals = [], [], []
        ...
        for i in range(len(player_data) - 1):
            ...
            actual = player_data.iloc[i + 1]['fantasy_points_ppr']
       ...
       # Split data into training and testing - remove last performance for later prediction
        train_size = int(0.8 * len(X_sequences))
        X_train, X_test = X_sequences[:train_size], X_sequences[train_size:-1]
        y_train, y_test = y_sequences[:train_size], y_sequences[train_size:-1]
        ...
        if espn_projection is None:
            logging.warning(f"ESPN projection not found for player: {player_name}, season: {current_season}, week: {current_week}")
        ...
        return jsonify({
            'message': f'Projection for {player_name}: {projection:.2f} fantasy points for Week {current_week} in {current_season}. \n {espn_projection_message}. Actual score: {actual_score}',
